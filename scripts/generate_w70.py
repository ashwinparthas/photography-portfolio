...
def generate_w70(path: Path) -> None:
    width, height = get_dimensions(path)
    out_path = path.with_name(f"{path.stem}-w70{path.suffix}")
    new_w = max(1, int(round(width * 0.7)))
    new_h = max(1, int(round(height * 0.7)))

    cmd = ["sips"]
    if path.suffix.lower() in {".jpg", ".jpeg"}:
        cmd += ["-s", "formatOptions", "70"]
...
def main() -> int:
    ...
    print(f"Generated {generated} w70 images.")
